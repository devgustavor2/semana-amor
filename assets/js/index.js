...
import { child, get, getDatabase, push, ref, set } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-database.js";
    ...
    function loadUserData() {
      get(child(ref(database), `users/${userId}/points`)).then((snapshot) => {
        if (snapshot.exists()) {
      ...
      get(child(ref(database), `users/${userId}/acertou`)).then((snapshot) => {
        if (snapshot.exists()) {
